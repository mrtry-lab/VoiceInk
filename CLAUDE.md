...
`upstream` に本家 [Beingpax/VoiceInk](https://github.com/Beingpax/VoiceInk) を設定している。本家の更新を取り込むときは upstream から fetch してマージする。fork 固有の変更（日本語ローカライズ、ダッシュボード解錠しきい値、`make signed` とローカル署名まわり）はこのリポジトリ側の差分として保持している。
